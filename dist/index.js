...
addEstudiante(e4);
addEstudiante(e5);
const addEstudiantePrin = (e) => {
    arregloEstudiante.unshift(e);
};
addEstudiantePrin(e2);
const actualizarApellidopornombre = (nombre, apellido) => {
...
// Ejemplo de uso
const encontrarBogotanos = (nuevaCiudad) => {
    const estudiantesBogotanos = arregloEstudiante.filter(estudiante => estudiante.addresses && estudiante.addresses.some(direccion => direccion.city.toLowerCase() === nuevaCiudad.toLowerCase()));
...
// Ejemplo de uso
const encontrarBogotanosmenos18 = (nuevaCiudad) => {
    const estudiantesBogotanos = arregloEstudiante.filter(estudiante => estudiante.addresses && estudiante.addresses.some(direccion => direccion.city.toLowerCase() === nuevaCiudad.toLowerCase()));
    // Cambiar la edad a 18 para los estudiantes encontrados
    estudiantesBogotanos.forEach(estudiante => {
        estudiante.edad = 18; // Asegúrate de que la propiedad "edad" exista en los estudiantes
...
const borrarEstudiante = (nombre, apellido) => {
    // Filtramos el arreglo para excluir al estudiante con el nombre y apellido especificados
    arregloEstudiante = arregloEstudiante.filter(estudiante => estudiante.nombre !== nombre || estudiante.apellido !== apellido);
};
// Ejemplo de uso
